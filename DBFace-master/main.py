"""
program name:基于pytorch模型的智慧识别检测系统
designer:zhaobin Cheng
功能：检测实物并进行识别
...
def image_demo():
    ...
    detect_image(dbface, "datas/cheng_picture3.jpg")
    detect_image(dbface, "datas/cheng_picture4.jpg")
...
def camera_demo():
    ...
    while ok:
        ...
        # cv2.namedWindow("detection show(cheng)",0);           #1.参数调整imshow窗口大小
        # cv2.resizeWindow("detection show(cheng)",1500,900);   #2.参数调整imshow窗口大小
        cv2.namedWindow("detection show(cheng)", 0);
        cv2.resizeWindow("detection show(cheng)", int(width*(height-80)/ pheight),height-80);
        cv2.imshow("detection show(cheng)", frame)
